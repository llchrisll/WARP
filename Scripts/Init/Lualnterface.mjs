...
export function load()
{
	...
	$$(_, 5.2, `Find where it is PUSHed. Should be followed by a CALL`)
	...
	{
		$$(_, 6.1, `Find the string '${fmt}' & save it's PUSH instruction`)
		const addr = Exe.FindText(fmt, false);
		if (addr > 0)
			Pushers.set(fmt, PUSH(addr));
	}

	$$(_, 6.2, `Set [Valid] to true`)
...
export function addLoaders(arg1, arg2, arg3, arg4)
{
	...
	$$(_, 1.3, `Find where it is PUSHed (it should be immediately followed by a CALL)`)
